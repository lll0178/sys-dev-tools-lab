...
> **说明**：本报告中各题"运行结果"均为在 WSL 中实际运行的输出；第 10 题的"失败测试"与第 12 题的损失值等关键过程均如实记录。

---

## 一、实验目的

1. 掌握 Python 包的标准打包流程（pyproject.toml、构建后端、入口点），并在干净虚拟环境中验证 wheel 的可安装性。
2. 掌握"测试驱动 + 编程智能体"的可验证修复循环，以及人工 diff 审查的必要性。
3. 掌握把模糊的协作材料（Issue / 提交信息 / 评审意见）改写成可执行信息的方法。
4. 掌握 PyTorch 线性回归训练循环的完整写法（梯度清零、反向传播、参数更新、评估模式）。

---

## 二、实验九：从源码构建并在干净环境安装 Wheel（q09）
...
（`src/greetlab/__init__.py` 为包标记文件。）
...
- `pyproject.toml` 是标准打包声明：`[build-system]` 指定构建后端（setuptools），`[project]` 提供元数据，`[project.scripts]` 把 `greetlab.cli:main` 注册为命令行入口 `sdt-greet`；
- **干净 venv + 只装 wheel** 模拟真实用户的安装路径，验证 wheel 自包含（不依赖源码目录、不依赖开发环境），这是发布前的"发货检查"；
- wheel 文件名 `greetlab_25030021043-0.1.0-py3-none-any.whl` 体现了分发名（连字符归一为下划线）、版本、Python 标签（py3）与平台标签（none-any = 纯 Python 跨平台）。

---

## 三、实验十：让编程智能体进入可验证的修复循环（q10）
...
- 这是 **TDD + 智能体闭环**：先用失败测试把"目标"变成可验证的断言（`SystemExit(2)`），再让智能体改实现，最后用同一条命令判定成败；
- **约束要显式给出**（不改语法、不加依赖），否则智能体可能"顺手"重构出无关改动；
- 人工 diff 审查是闭环的最后一道闸门：确认改动最小、无副作用后再合并。

---

## 四、实验十一：把"无法处理"的协作材料改成可执行信息（q11）
...
- 原始材料"不可执行"：`Windows 上运行不了` 缺环境与复现步骤，`fix bug` 无信息量，`写得不好，重写` 无可操作动作；
- 重写标准是**接手人能否直接照做**：Issue 给环境 + 复现 + 期望/实际，未知项显式标注"待确认"；提交信息用祈使句标题 + 问题与方案正文；评审意见给出具体行为、风险、建议动作并标注 Blocking/Suggestion/Nit 级别。

---

## 五、实验十二：修复一个可复现的线性回归训练循环（q12）

### 5.1 补全后的代码（q12/train.py）
...
- 训练循环三要素缺一不可：`zero_grad()` 清空上一轮梯度（否则梯度会累积）、`backward()` 反向传播求梯度、`step()` 按优化器规则更新参数；
- 评估阶段用 `model.eval()` + `torch.no_grad()`：前者切换评估态（影响 Dropout/BatchNorm 等），后者不构建计算图，省内存也更快；
- 最终 `weight ≈ 3`、`bias ≈ -1` 是 200 轮梯度下降**训练收敛**的结果（`torch.manual_seed(20260907)` 保证可复现），并非直接赋值；最终损失 $0 < 0.001$，满足要求。
...
| 智能体可能引入无关改动 | 缺少显式约束 | 在提示中写明约束（不改语法/不加依赖），并人工 diff 审查 |
